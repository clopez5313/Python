class Stack:
    ...
    def isBalanced(self, items):
        # If the items list doesn't have an even number of characters, it's not balanced.
        if (len(items) %2 != 0):
            return "The stack is not balanced."

        # Check the items that want to be pushed into the stack.
        for item in items:
            ...
            if(not self.is_empty()):
                if((")" in item and self.peek() == "(") or ("]" in item and self.peek() == "[") or ("}" in item and self.peek() == "{")):
                    self.pop()
                    continue
